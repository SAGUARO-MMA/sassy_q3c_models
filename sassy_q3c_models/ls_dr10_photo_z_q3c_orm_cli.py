...
# import(s)
# -
from sassy_q3c_models.ls_dr10_photo_z_q3c_orm_filters import *


# +
# __doc__ string
# -
__doc__ = """ python3 ls_dr10_photo_z_q3c_orm_cli.py --help """


# +
# constant(s)
# -
ARXIV_PDF_URL = LSDR10_PHOTOZ_PDF_URL
ARXIV_PDF_FIL = LSDR10_PHOTOZ_PDF_URL.split("/")[-1]
ARXIV_DAT_URL = LSDR10_PHOTOZ_DAT_URL
ARXIV_DAT_FIL = LSDR10_PHOTOZ_DAT_URL.split("/")[-1]
...
# +
# function: ls_dr10_photo_z_q3c_orm_cli()
# -
# noinspection PyBroadException
def ls_dr10_photo_z_q3c_orm_cli(_args: Any = None):
    ...
    try:
        ...
        if _args.verbose:
            print(f'query = {query}')
        query = ls_dr10_photo_z_q3c_orm_filters(query, request_args)
        if _args.verbose:
            print(f'query = {query}')
    ...
    print(f"#{','.join(_ for _ in LSDR10_PHOTOZ_HEADERS)}")
    for _e in LsDr10PhotoZQ3cRecord.serialize_list(query.all()):
        if verify_keys(_e, set(LSDR10_PHOTOZ_HEADERS)):
            print(f"{','.join(str(_e[_l]) for _l in LSDR10_PHOTOZ_HEADERS)}")
...
if __name__ == '__main__':
    ...
    _p = argparse.ArgumentParser(description=f'Query LSDR10_PHOTOZ Q3C database', formatter_class=argparse.RawTextHelpFormatter)
    ...
    # execute
    try:
        ls_dr10_photo_z_q3c_orm_cli(_args=_a)
