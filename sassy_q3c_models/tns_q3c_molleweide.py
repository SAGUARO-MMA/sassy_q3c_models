...
# __doc__
# -
__doc__ = """ python3 tns_q3c_molleweide.py --help """


# +
# function: tns_q3c_molleweide()
# -
def tns_q3c_molleweide(_verbose: bool = False):
    ...
    ax.grid(True)
    plt.title(f"Transient Name Server Coverage Map")

    # output(s)
    _png = os.path.abspath(os.path.expanduser(f"tns_q3c_molleweide.png"))
...
if __name__ == '__main__':
    ...
    _p = argparse.ArgumentParser(description='Plot TNS Molleweide', formatter_class=argparse.RawTextHelpFormatter)
    _p.add_argument(f'--verbose', default=False, action='store_true', help='if present, produce more verbose output')
    _a = _p.parse_args()

    # execute
    try:
        tns_q3c_molleweide(_verbose=bool(_a.verbose))
