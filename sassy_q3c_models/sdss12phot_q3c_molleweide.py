...
# __doc__
# -
__doc__ = """ python3 sdss12phot_q3c_molleweide.py --help """


# +
# function: sdss12phot_q3c_molleweide()
# -
def sdss12phot_q3c_molleweide(_verbose: bool = False):
    ...
    try:
        ax.scatter(np.radians(_ra_np), np.radians(_dec_np), color='blue', s=2.5,
                   alpha=0.25, marker='D', label="SDSS12PHOT")
    ...
    ax.grid(True)
    plt.title(f"SDSS12PHOT Coverage Map")

    # output(s)
    _png = os.path.abspath(os.path.expanduser(f"sdss12phot_q3c_molleweide.png"))
...
if __name__ == '__main__':
    ...
    _p = argparse.ArgumentParser(description='Plot SDSS12PHOT Molleweide', formatter_class=argparse.RawTextHelpFormatter)
    _p.add_argument(f'--verbose', default=False, action='store_true', help='if present, produce more verbose output')
    _a = _p.parse_args()

    # execute
    try:
        sdss12phot_q3c_molleweide(_verbose=bool(_a.verbose))
