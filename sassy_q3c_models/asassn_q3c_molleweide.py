...
# __doc__
# -
__doc__ = """ python3 asassn_q3c_molleweide.py --help """


# +
# function: asassn_q3c_molleweide()
# -
def asassn_q3c_molleweide(_verbose: bool = False):
    ...
    try:
        ax.scatter(np.radians(_ra_np), np.radians(_dec_np), color='blue', s=2.5,
                   alpha=0.25, marker='D', label="ASASSN")
    ...
    ax.grid(True)
    plt.title(f"ASASSN Coverage Map")

    # output(s)
    _png = os.path.abspath(os.path.expanduser(f"asassn_q3c_molleweide.png"))
...
if __name__ == '__main__':
    ...
    _p = argparse.ArgumentParser(description='Plot ASASSN Molleweide', formatter_class=argparse.RawTextHelpFormatter)
    _p.add_argument(f'--verbose', default=False, action='store_true', help='if present, produce more verbose output')
    _a = _p.parse_args()

    # execute
    try:
        asassn_q3c_molleweide(_verbose=bool(_a.verbose))
